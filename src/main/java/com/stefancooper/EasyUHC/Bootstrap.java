...
public class Bootstrap implements PluginBootstrap {
    ...
    public void bootstrap(BootstrapContext context) {
        ...
        RegistryEvents.ENCHANTMENT.compose().newHandler(event -> {
            try {
                final TypedKey<Enchantment> quickboomKey = EnchantmentKeys.create(Key.key(String.format("%s:%s", Constants.NAMESPACE, Constants.QUICKBOOM_ENCHANTMENT)));
                final TypedKey<Enchantment> blastwaveKey = EnchantmentKeys.create(Key.key(String.format("%s:%s", Constants.NAMESPACE, Constants.BLASTWAVE_ENCHANTMENT)));
